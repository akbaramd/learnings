...
import { SubmitResponseResponseWrapper } from '@/src/store/surveys/surveys.types';
...
export async function POST(
  ...
  try {
    ...
    const upstream = await api.api.submitResponse(surveyId, responseId, {});
    
    const status = upstream.status ?? 200;

    const response: SubmitResponseResponseWrapper = {
