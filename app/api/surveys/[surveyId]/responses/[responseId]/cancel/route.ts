...
import { CancelResponseResponseWrapper } from '@/src/store/surveys/surveys.types';
import { AxiosError } from 'axios';

/**
 * POST /api/surveys/[surveyId]/responses/[responseId]/cancel
 * Cancel survey response
 */
export async function POST(
  ...
  try {
    ...
    const upstream = await api.api.cancelResponse(surveyId, responseId, {});
    
    const status = upstream.status ?? 200;

    const response: CancelResponseResponseWrapper = {
