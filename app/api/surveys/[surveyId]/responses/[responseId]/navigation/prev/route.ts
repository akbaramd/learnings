...
import { GoPreviousQuestionResponseWrapper } from '@/src/store/surveys/surveys.types';
import { AxiosError } from 'axios';

/**
 * POST /api/surveys/[surveyId]/responses/[responseId]/navigation/prev
...
export async function POST(
  ...
  try {
    ...
    const upstream = await api.api.goPreviousQuestion(surveyId, responseId, {});
    
    const status = upstream.status ?? 200;

    const response: GoPreviousQuestionResponseWrapper = {
