...
import { GetResponseProgressResponse } from '@/src/store/surveys/surveys.types';
import { AxiosError } from 'axios';

/**
 * GET /api/surveys/[surveyId]/responses/[responseId]/progress
...
export async function GET(
  ...
  try {
    ...
    const upstream = await api.api.getResponseProgress(surveyId, responseId, {});
    
    const status = upstream.status ?? 200;

    const response: GetResponseProgressResponse = {
